...
load('~/Downloads/dummy.rda') 
# make path on next line the fodler with all the json files
#list.files("~/Downloads/KKD")
filenames <- list.files("~/Downloads/FBL", pattern="*.json", full.names=TRUE)
...
matches <- read.csv("/Users/marclamberts/Downloads/FBL Matches.csv") %>%
...
for(i in 1:length(filenames)){
  ...
  write.csv(Wedstrijd, glue::glue('~/Downloads/FBL/xgCSV/{gsub(".json","",str_split_fixed(filenames[i], n=6,pattern = "/")[6])}.csv'))
  #######

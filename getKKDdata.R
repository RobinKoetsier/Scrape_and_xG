...
filenames <- list.files("~/Documents/ScraperWhoScored/KKD", pattern=glob2rx("*.json"), full.names=TRUE)
...
#for(i in 276:276){
  ...
  game_wide2 <- game_wide2 %>%
    mutate(match =
    gsub(".json","",str_split_fixed(filenames[i], n=7,pattern = "/")[7])) %>%
    mutate(match =
             gsub(".*_","",match)) %>%
    mutate(match= 
 ...
 # write.csv(df_rel %>% 
  ...
  #write.csv(Wedstrijd, glue::glue('~/Documents/ScraperWhoScored/KKD/xgCSV/{gsub(".json","",str_split_fixed(filenames[i], n=7,pattern = "/")[7])}.csv'))
  #######

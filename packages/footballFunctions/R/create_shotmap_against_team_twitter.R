...
#' @param name_of_player Name of the team defined in the excel file, default = Ajax.
...
create_shotmap_against_team_twitter <- function(name_of_team = "Ajax", showstats = FALSE){
  df <- readxl::read_excel("Export_TDL_NED_2122.xlsx", 
                           sheet = "Shots")%>%
    
    separate(match, c("Home", "Away"), " - ") %>%
    filter((name_of_team == Home |
             name_of_team == Away)& name_of_team != name) %>%
  ...
  p <- ggplot(df ,aes(x,y)) +
    ...
    coord_flip(xlim=c(50,100),
               ylim=c(100,0))+
    scale_fill_manual(values=c("0" = "#120E41","1"= "pink"),labels=c("Miss","Goal")) +
    scale_color_manual(values=c("0" = "pink","1" = "#120E41"),labels=c("Miss","Goal")) +
  ...
  if(showstats == TRUE){
    p <- p+ 
      ...
      labs(x="",
           ...
           fill = " ",
           colour=" ",
           title = glue::glue("Shotmap against {name_of_team} Eredivisie 21/22"),

#' A plot function
#'makes a shotmap of a Eredivisie Team
...
create_shotmap_team_twitter <- function(name_of_team = "Ajax", showstats = FALSE){
  df <- readxl::read_excel("Export_TDL_NED_2122.xlsx", 
                           sheet = "Shots")%>%
    filter(OwnGoal == FALSE) %>%
    filter(name %in% name_of_team) %>%
  ...
  if(showstats == TRUE){
    p <- p+ 
      ...
      labs(x="",
           ...
           fill = " ",
           colour=" ",
           title = glue::glue("Shotmap {name_of_team} Eredivisie 21/22"),

...
PrepAGame <- function(df){
  ...
  isIntentionalAssist <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bIntentionalAssist\\b", .x))) > 0) %>% mutate(isIntentionalAssist = "Yes") %>% select(PlayerId,id,isIntentionalAssist)
  ...
  Left <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bLeftFoot\\b", .x))) > 0)  %>% mutate(Bodypart = "LeftFoot")  %>% select(PlayerId,id,Bodypart)
  Right <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bRightFoot\\b", .x))) > 0) %>% mutate(Bodypart = "RightFoot")  %>% select(PlayerId,id,Bodypart)
  Other <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bOtherBodyPart\\b", .x))) > 0)  %>% mutate(Bodypart = "OtherBP")  %>% select(PlayerId,id,Bodypart)
  ...
  OutofBoxCentre <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bOutOfBoxCentre\\b", .x))) > 0) %>% mutate(zone = "OutOfBoxCentre") %>% select(PlayerId,id,zone)
  OutofBoxLeft <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bOutofBoxLeft\\b", .x))) > 0)  %>% mutate(zone = "OutofBoxLeft") %>% select(PlayerId,id,zone)
  OutofBoxRight <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bOutofBoxRight\\b", .x))) > 0) %>% mutate(zone = "OutofBoxRight") %>% select(PlayerId,id,zone)
  ...
  BoxLeft <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bBoxLeft\\b", .x))) > 0) %>% mutate(zone = "BoxLeft") %>% select(PlayerId,id,zone)
  BoxRight <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bBoxRight\\b", .x))) > 0) %>% mutate(zone = "BoxRight") %>% select(PlayerId,id,zone)
  BoxCentre <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bBoxCentre\\b", .x))) > 0) %>% mutate(zone = "BoxCentre") %>% select(PlayerId,id,zone)
  ...
  DeepBoxLeft <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bDeepBoxLeft\\b", .x))) > 0) %>% mutate(zone = "DeepBoxLeft") %>% select(PlayerId,id,zone)
  DeepBoxRight <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bDeepBoxRight\\b", .x))) > 0) %>% mutate(zone = "DeepBoxRight") %>% select(PlayerId,id,zone)
  DeepBoxCentre <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bDeepBoxCentre\\b", .x))) > 0) %>% mutate(zone = "DeepBoxCentre") %>% select(PlayerId,id,zone)
  ...
  SmallBoxLeft <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bSmallBoxLeft\\b", .x))) > 0) %>% mutate(zone = "SmallBoxLeft") %>% select(PlayerId,id,zone)
  SmallBoxRight <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bSmallBoxRight\\b", .x))) > 0) %>% mutate(zone = "SmallBoxRight") %>% select(PlayerId,id,zone)
  SmallBoxCentre <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bSmallBoxCentre\\b", .x))) > 0) %>% mutate(zone = "SmallBoxCentre") %>% select(PlayerId,id,zone)
  ...
  ThirtyFivePlusLeft <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bThirtyFivePlusLeft\\b", .x))) > 0) %>% mutate(zone = "ThirtyFivePlusLeft") %>% select(PlayerId,id,zone)
  ThirtyFivePlusRight <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bThirtyFivePlusRight\\b", .x))) > 0) %>% mutate(zone = "ThirtyFivePlusRight") %>% select(PlayerId,id,zone)
  ThirtyFivePlusCentre <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bThirtyFivePlusCentre\\b", .x))) > 0) %>% mutate(zone = "ThirtyFivePlusCentre") %>% select(PlayerId,id,zone)
  ...
  BigChance <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bBigChance\\b", .x))) > 0) %>% mutate(isBigChance = "Yes") %>% select(PlayerId,id,isBigChance)
...
#                DeepBoxLeft,DeepBoxRight,DeepBoxCentre)
  ...
  FromCorner <- OneGame %>% filter(rowSums(across(everything(), ~grepl("FromCorner", .x))) > 0) %>% mutate(Type_of_play = "FromCorner") %>% select(PlayerId,id,Type_of_play)          
  RegularPlay <- OneGame %>% filter(rowSums(across(everything(), ~grepl("RegularPlay", .x))) > 0) %>% mutate(Type_of_play = "RegularPlay") %>% select(PlayerId,id,Type_of_play)          
  DirectFreekick <- OneGame %>% filter(rowSums(across(everything(), ~grepl("DirectFreekick", .x))) > 0) %>% mutate(Type_of_play = "DirectFreekick") %>% select(PlayerId,id,Type_of_play)          
  SetPiece <- OneGame %>% filter(rowSums(across(everything(), ~grepl("\\bSetPiece\\b", .x))) > 0) %>% mutate(Type_of_play = "SetPiece") %>% select(PlayerId,id,Type_of_play)          
  FastBreak <- OneGame %>% filter(rowSums(across(everything(), ~grepl("FastBreak", .x))) > 0) %>% mutate(Type_of_play = "FastBreak") %>% select(PlayerId,id,Type_of_play)          
  ThrowinSetPiece <- OneGame %>% filter(rowSums(across(everything(), ~grepl("ThrowinSetPiece", .x))) > 0) %>% mutate(Type_of_play = "ThrowinSetPiece") %>% select(PlayerId,id,Type_of_play)          
  ...
  shots_ext <- df %>% 
    ...
    mutate(angle = goal_angle(x, y)) %>%# based on available goal mouth
    select(Date,PlayerId,TeamId,HomeTeam,AwayTeam,minute,second,x,y,angle,distance,Type_of_play,
           ...
           GamestateOne,GamestateTwo,Bodypart,TeamId,isGoal,isBigChance,relatedPlayerId,
           isIntentionalAssist,isAssistedShot,isOwnGoal, goalMouthY, goalMouthZ,expandedMinute) %>%
    mutate(Goal = ifelse(isGoal == "TRUE",1,0)) %>%
    mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>%

#' Needed for CalculateChance()
#' 
#' @param wedstrijd data frame
#' @return kansen
#' @examples
#' GetWedstrijden(wedstrijd)
#' @export
...
GetWedstrijden <- function(Wedstrijd){
  ...
  kansen <- data.frame(Wedstrijd$Date[1],Wedstrijd$Home[1],sum(Home$Goal),
    ...
    set_colnames(c("Datum","ThuisTeam","Thuisgoals","Uitgoals","Uitteam","xGThuis","xGUit","Kans winst thuisteam",
                   "Kans gelijkspel","Kans winst Uitteam","xpHome","xpAway"))
...
#' 
#' @param team1 Home team xg vector
#' @param team2 Away team xg vector
#' @param p Number of sims
#' @return chance
#' @examples
#' calculateChance(team1,team2,p)
#' @export
...
#' Needed for CalculateChance()
#' 
#' @param home home team xg
#' @param away away team xg
#' @return winner
#' @examples

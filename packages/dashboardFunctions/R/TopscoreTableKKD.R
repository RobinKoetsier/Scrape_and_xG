...
TopscoreTableKKD <- function(){
  library(gt)
  df <- readxl::read_excel("Export_TDL_KKD_2223.xlsx", 
  ...
  df %>%
    ...
    tab_header(
      title = md("**Topscorers Eerste Divisie 2022/2023**"),
      subtitle = "Sorted on goals and xG"
    )%>%
    tab_source_note(md("**Twitter**: @EersteDivPlots | **Site**: EredivisiePlots.com")
    )  %>%
    opt_table_font(font = "Roboto Mono"
    ) %>%
    gtsave(filename = "plots/TopscorersKKD.png",vwidth = 1000, zoom = 6)

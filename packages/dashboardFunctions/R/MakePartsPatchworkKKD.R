...
MakePartsPatchworkKKD <- function(listEntry1,listEntry2, listEntry3 ){
  library(ggsoccer)
  library(ggtext)
  library(tidyverse)
  library(grid)
  ...
  Kleur1 <- "#15607A"
  Kleur2 <- "#65E0BA"
  Wedstrijd = data.frame(listEntry1)
  df_rel <- data.frame(listEntry2)
  OneGame <- data.frame(listEntry2)
  With_names <<- data.frame(listEntry3)
  ...
  if(HomeGoals == 0 &
    ...
    shotmap <- ggplot() +annotate_pitch(fill="#f2f4f5")+
      ...
      annotate(geom="text",x=25,y=122,label=glue::glue("{Wedstrijd$HomeTeam[1]}"), 
               family="Spartan-Bold",colour=Kleur1,size=6 ,colour="white") +
      ...
      annotate(geom="text",x=75,y=122,label=glue::glue("{Wedstrijd$AwayTeam[1]}"), 
               family="Spartan-Bold",colour=Kleur2,size=6 ) +
    ...
    shotmap <- ggplot() +annotate_pitch(fill="#f2f4f5")+
      ...
      xlim(0,100)+
      #scale_size(range = c(0, 10))+
      annotate(geom="label",x=25,y=122,label=glue::glue("{Wedstrijd$HomeTeam[1]}"), 
               family="Spartan-Bold",fill=Kleur1,size=6,colour="white" ) +
      annotate(geom="text",x=25,y=116,label=glue::glue("{HomeGoals}"), 
               family="Spartan-Bold",colour="#362626",size=5 ) +
      annotate(geom="text",x=25,y=110,label=glue::glue("{xGLabelHome}"), 
               family="Spartan-Bold",colour="#362626",size=5 ) +
      ...
      annotate(geom="text",x=50,y=104,label=glue::glue("Winning chance"), 
               family="Spartan-Bold",colour="#362626",size=5 )+
      annotate(geom="label",x=75,y=122,label=glue::glue("{Wedstrijd$AwayTeam[1]}"), 
               family="Spartan-Bold",fill=Kleur2,size=6 ) +
      annotate(geom="text",x=75,y=116,label=glue::glue("{AwayGoals}"), 
               family="Spartan-Bold",colour="#362626",size=5 ) +
      annotate(geom="text",x=75,y=110,label=glue::glue("{xGLabelAway}"), 
               family="Spartan-Bold",colour="#362626",size=5 ) +
  ...
  timeline <- ggplot() + 
    ...
    theme(plot.background = element_rect(fill = "#f2f4f5", colour = "#f2f4f5"), 
          ...
          plot.subtitle = element_markdown(hjust=0.5,family="Spartan-Light",color="#f2f4f5",size=8),
          plot.caption = element_text(size=8,family="Spartan-Medium",color="Green4"))
  OneGame <- data.frame(OneGame)
  redCard <- OneGame %>% 
    dplyr::filter((rowSums(across(everything(), ~grepl("Red card", .x))) > 0),
                  (rowSums(across(everything(), ~grepl("Second yellow", .x))) > 0)) %>%
    select(timeMin)
  if(nrow(redCard)>0){
    timeline = timeline + 
  ...
  xT <- df_rel %>% 
    ...
    ungroup() %>%
     mutate(side = ifelse(TeamId == HomeTeam[1],"Home","Away"))
  ...
  PassReady <- OneGame %>% 
    left_join(getEndKKD(With_names)) %>% rename(PlayerId = playerName)
  PassReady$xm <- PassReady$x * 106
  PassReady$xendm <- PassReady$endX * 106
  PassReady$ym <- PassReady$y * 68
  PassReady$yendm <- PassReady$endY * 68
  PassReady$startToGoal <- sqrt((10600 - PassReady$xm)^2 + (3400 - PassReady$ym)^2)
  PassReady$endToGoal <- sqrt((10600 - PassReady$xendm)^2 + (3400 - PassReady$yendm)^2)
  ...
  keypassdf <- PassReady%>% group_by(PlayerId,TeamId,side) %>% filter(progressive == "yes") %>%
    mutate(Succ = ifelse(outcome == 1,1,0))%>%
  ...
  dev.off()
p
  ggsave(glue::glue("Graphs/dashboard1.png"),width=15,height=10,device=png) 
}
